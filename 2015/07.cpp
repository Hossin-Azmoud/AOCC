...
uint16_t execute_operation(std::map<std::string, uint16_t> &Ram, 
  ...
  if (instruction.lhs) {
    if (isdigits(instruction.lhs)) {
      ...
      if (auto search = Ram.find(instruction.lhs); search != Ram.end()) {
        lhs = search->second;
      } else {
        if (auto search = instructions.find(instruction.lhs); search != instructions.end())
        {
          lhs = execute_operation(Ram, search->second, instructions);
        } else {
          fprintf(stderr, "Undefined var\n");
          exit(1);
        }
      }
    }
  ...
  if (instruction.rhs) {
    if (isdigits(instruction.rhs)) {
      ...
      // Get the value from the map
      if (auto search = Ram.find(instruction.rhs); search != Ram.end()) {
        rhs = search->second;
      } else {
        if (auto search = instructions.find(instruction.rhs); search != instructions.end())
        {
          rhs = execute_operation(Ram, search->second, instructions);
        } else {
          fprintf(stderr, "Undefined var\n");
          exit(1);
        }
  ...
  // insert(Ram, instruction.dst, result);
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < ip; i++) {
    execute_operation(ram0, ops[i], prog);
  }
  for (auto it = ram0.begin(); it != ram0.end(); it++) {
    auto k = it->first;
    auto v = it->second;
    printf("%s: %u\n", k.c_str(), v);
  }
